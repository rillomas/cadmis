...
// */
        ...
        for j := range items2 {
            ...
            key, err2 := datastore.Put(c, datastore.NewIncompleteKey(c, "Problem", nil), &problem)
            if err2 != nil {
                http.Error(w, err2.Error(), http.StatusInternalServerError)
                return
            }
...
func getExamResultScore(c appengine.Context, e ExamResult) (error, int32) {
    var score int32
    for i := range e.ProblemList {
        ...
        if (pr.Correct) {
            q := datastore.NewQuery("Problem").Filter("Id =", pr.ProblemId).Limit(1)
...
func getExamScore(c appengine.Context, e Exam) (error, int32) {
    var score int32
    for i := range e.ProblemList {
        p := new(Problem)
